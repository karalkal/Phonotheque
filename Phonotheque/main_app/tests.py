...
class AlbumArtistCollectionTests(django_test.TestCase):
    VALID_USER_DATA = {
        'username': "test_user",
        'password': '11111111',
        'email': "test@user.com",
    ...
    def __create_comment_for_user(self, user):
        comment = Comment.objects.create(**self.VALID_COMMENT, user=user)
        return comment
    ...
    def test_when_user_has_no_collection_items__collection_must_be_empty(self):
    ...
    def test_when_user_has__collection_items__collection_must_contain_album(self):
        user = self.__create_valid_user()
    ...
    def test_when_album__is_created__artist_must_be_created_too(self):
        album = self.__create_album(**self.VALID_ALBUM)
        artist = Artist.objects.get(pk=album.artist.pk)
        self.assertEqual(artist, album.artist)

    def test_when_album__is_already_created_and_another_users_adds_to_their_collection__no_error_and_both_must_have_it(
            self):
        album = self.__create_album(**self.VALID_ALBUM)
    ...
    def test_when_artist__is_already_saved__no_error_must_be_raised_albums_must_be_saved_and_related_to_same_artist(
            self):
        artist = self.__create_artist()
